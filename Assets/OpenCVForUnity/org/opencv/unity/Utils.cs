...
namespace OpenCVForUnity.UnityUtils
{
    public static class Utils
    {
        ...
        */
        ...
        public static void RawTextureDataToMat(NativeArray<byte> data, Mat mat, int width, int height, bool flip = true, int flipCode = 0)
        {
            if (data == null)
            ...
            unsafe
            {
                OpenCVForUnity_ByteArrayToMatData((IntPtr)data.GetUnsafeReadOnlyPtr(), mat.nativeObj);
            }
            if (flip)
            {
                Core.flip(mat, mat, flipCode);
